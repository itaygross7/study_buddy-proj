...
APP_HELP_RESPONSES = {
    ...
    # Library
    "library": "📚 **הספרייה** - כאן אתה יוצר קורסים ומעלה חומרים. כל קורס מופרד - אבנר עונה רק מהחומר של הקורס הספציפי.",
    "course": "📖 **קורס** - צור קורס לכל נושא (למשל: מבוא לפסיכולוגיה). העלה אליו חומרים והשתמש בכלים.",
    "upload": "📤 **העלאת חומר** - אפשר להעלות PDF, Word, PowerPoint או להדביק טקסט. החומר נשמר רק בקורס שלך.",
...
def get_local_response(question_type: str, key: str, question: str) -> str:
    ...
    if question_type == 'offtopic':
        return "🦫 אני מתמחה בעזרה בלימודים ובשימוש באפליקציה. אם יש לך שאלה על החומר שהעלית או על איך להשתמש ב-StudyBuddy - אשמח לעזור!"
